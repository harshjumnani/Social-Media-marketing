...
<div class="container-fluid">
   ...
   <ul class="nav nav-tabs nav-tabs__service">
      ...
      <li class="p-b"><button class="btn btn-default m-l" data-toggle="modal" data-target="#modalDiv" data-action="new_subscriptions">ADD SUBSCRIPTIONS</button></li>
      ...
      <li class="pull-right">
        <a data-toggle="modal" data-target="#modalDiv" data-action="import_services">IMPORT FROM API</a>
      </li>
   </ul>
                                       ...
                                       <tr id="serviceshowcategory-<?=$c?>" class="ui-state-default <?php if( $services[$i]["service_type"] == 1 ): echo "grey"; endif; ?>"  data-category="category-<?=$c?>" data-id="service-<?php echo $services[$i]["service_id"] ?>" data-service="<?php echo $services[$i]["service_name"] ?>">
                                         <?php if( !empty($services[0]["service_id"])  ):?>
                                            ...
                                            <td class="service-block__rate">
                                              ...
                                              ?>
                                               <div style="<?php if( !$api_detail["rate"] ): echo ""; elseif( $services[$i]["service_api"] != 0 && $services[$i]["service_price"] > $api_price*$settings["inr_charge"]): echo "color: green"; elseif( $services[$i]["service_api"] != 0 && $services[$i]["service_price"] < $api_price*$settings["inr_charge"]): echo "color: red"; endif ?>"><?php echo round($services[$i]["service_price"],3) ?></div>

                                               <?php if( $services[$i]["service_api"] != 0 && $api_detail["rate"] ): echo '<div class="service-block__provider-value"> '.priceFormat($api_detail["rate"]*$settings["inr_charge"]).' ₹
                                            
                                            </div>'; endif; ?>
                                             <!--<i class="fa fa-'.strtolower($api_detail["currency"]).'"></i>-->
                                            ...
                                            <td class="service-block__visibility"><?php if( $services[$i]["service_type"] == 1 ): echo "Passive "; else: echo "Active"; endif; ?> <?php if($services[$i]["api_servicetype"]==1): echo '<span class="text-danger" title="Service provider has removed the service"><span class="fa fa-exclamation-circle"></span></span>'; endif;?> </td>
